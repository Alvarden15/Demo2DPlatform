...
class Design_42_42_GameDebugger extends SceneScript
{
	...
	override public function init()
	{
				...
				if(_Enabled)
				{
					...
					if((_DrawOrientation || (_DrawVelocity || _DrawActorIDs)))
					{
						...
						for(actorOnScreen in engine.allActors)
						{
							...
							{
								if(!((_ExcludeLightweightActors && actorOnScreen.physicsMode != NORMAL_PHYSICS)))
								{
									g.strokeSize = Std.int(_StrokeThickness);
									_XCenterOnScreen = (actorOnScreen.getXCenter() - getScreenX());
									_YCenterOnScreen = (actorOnScreen.getYCenter() - getScreenY());
									if(_DrawVelocity)
									{
									...
									if(_DrawActorIDs)
									{
										...
										g.drawString("" + _Text, ((_XCenterOnScreen - (g.font.font.getTextWidth(_Text, g.font.letterSpacing)/Engine.SCALE / 2)) + _ActorIDXOffset), ((_YCenterOnScreen - (g.font.getHeight()/Engine.SCALE / 2)) + _ActorIDYOffset));
									}
								}
							}
						}
		...
		addKeyStateListener(_IncreaseGameSpeedControl, function(pressed:Bool, released:Bool, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && released)
			{
				_StepSize = Math.max((_StepSize - 1), 1);
